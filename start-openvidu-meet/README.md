...
2. Building the project with the repository's own workspace configuration
3. Optionally starting the backend in CI mode (configurable)
...
| `skip_backend_start`       | Skip starting the backend server (only build the project)                                        | No       | `false`  |
...
- When `skip_backend_start` is `false` (default):
  ...
  - The backend is started using `./meet.sh start --ci --skip-install` in background mode
